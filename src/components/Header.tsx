    ...
    <Stack spacing={2}>
      ...
      <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap">
        ...
        <TextField
          ...
          onChange={(e) => dispatch(setServingsPerPerson(Math.max(1, Math.min(8, Math.trunc(Number(e.target.value) || 1)))))}
        ...
        <TextField
          ...
          type="number"
          value={people}
          onChange={(e) => dispatch(setPeople(Math.max(1, Math.min(32, Math.trunc(Number(e.target.value) || 1)))))}
          inputProps={{ min: 1, step: 1, 'aria-label': 'people' }}
          sx={{ maxWidth: 110 }}
        />
        <Button variant="contained" onClick={handleSave}>Сохранить меню</Button>
        <Button color="warning" onClick={() => dispatch(resetWeek())}>Очистить неделю</Button>
        <Button color="error" onClick={handleResetAll}>Сбросить всё</Button>
